...
import App from './AdvancedWorkspace'
...
it('lets a human edit a next action, preview a scoped AI handoff and find the idea later', async () => {
  const user = userEvent.setup()
  const repository = createRepository()
  repository.captureSeed({ text: 'Prepare an interview' })
  repository.captureSeed({ text: 'Unrelated private task' })
  render(<App repository={repository} />)
  const card = screen.getByRole('heading', {name: 'Prepare an interview'}).closest('li')!
  await user.click(within(card).getByRole('button', {name: 'Edit idea'}))
  await user.type(screen.getByLabelText('Next action'), 'Practice one STAR story')
  await user.click(screen.getByRole('button', {name: 'Save idea'}))
  expect(repository.getState().seeds.find(seed => seed.text === 'Prepare an interview')?.nextAction).toBe('Practice one STAR story')
  await user.click(within(card).getByRole('button', {name: 'AI handoff'}))
  expect((screen.getByLabelText('Handoff preview') as HTMLTextAreaElement).value).toContain('Practice one STAR story')
  expect((screen.getByLabelText('Handoff preview') as HTMLTextAreaElement).value).not.toContain('Unrelated private task')
  await user.click(within(screen.getByRole('navigation', {name: 'Operate'})).getByRole('button', {name: 'Review'}))
  await user.type(screen.getByLabelText('Search ideas'), 'STAR')
  const review = screen.getByRole('region', {name: 'Review'})
  expect(within(review).getByRole('heading', {name: 'Prepare an interview'})).toBeInTheDocument()
  expect(within(review).queryByRole('heading', {name: 'Unrelated private task'})).not.toBeInTheDocument()
})

it('shows recorded outcomes after a focus block finishes', async () => {
  const user = userEvent.setup()
  const repository = createRepository()
  const seed = repository.captureSeed({text: 'Prepare for interview'})
  repository.startFocusSession(seed.id, 5)
  repository.completeFocusSession(repository.getActiveFocusSession()!.id, 'Rehearsed my modernization story')
  render(<App repository={repository} />)
  await user.click(within(screen.getByRole('navigation', {name: 'Operate'})).getByRole('button', {name: 'Focus'}))
  expect(within(screen.getByRole('region', {name: 'Recent outcomes'})).getByText('Rehearsed my modernization story')).toBeInTheDocument()
})

it('keeps the concrete next action visible during focus and can archive finished work from Review', async () => {
  const user = userEvent.setup()
  const repository = createRepository()
  const seed = repository.captureSeed({text: 'Interview preparation'})
  repository.editSeed(seed.id, {text: seed.text, note: '', nextAction: 'Practice one story aloud'})
  render(<App repository={repository} />)
  await user.click(screen.getByRole('button', {name: 'Focus now'}))
  expect(within(screen.getByRole('region', {name: 'Focus'})).getByText('Practice one story aloud')).toBeInTheDocument()
  await user.click(screen.getByRole('button', {name: 'Complete'}))
  await user.click(within(screen.getByRole('navigation', {name: 'Operate'})).getByRole('button', {name: 'Review'}))
  const review = screen.getByRole('region', {name: 'Review'})
  await user.click(within(review).getByRole('button', {name: 'Archive'}))
  expect(repository.getSeed(seed.id)?.status).toBe('archived')
  await user.click(within(review).getByRole('button', {name: 'Restore'}))
  expect(repository.getSeed(seed.id)?.status).toBe('inbox')
})
